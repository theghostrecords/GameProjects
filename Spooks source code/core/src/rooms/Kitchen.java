...
public class Kitchen implements Screen {
	...
	public Kitchen(Spooks games, Stage stages, Hud huds, SpooksGame gameState) {
		
		Spooks.manager.get("sound/rain.wav", Music.class).setPan(-1.0f, 0.2f);
		...
		this.game = games;
		this.stage = stages;
		this.map = new Texture("background\\kitchen.png");
		...
		roomChange.setPosition(Spooks.width / 15, Spooks.height / 5);
		...
		roomChange.addListener(new ClickListener() {
			...
			public void enter(InputEvent event, float x, float y, int pointer, Actor fromActor) {
				...
				arrow.setPosition(Spooks.width / 15 + 128, Spooks.height / 5 + 128);
		...
		AddInvisibleObject lighter = new AddInvisibleObject(game, stage, hud, 1150, 400, "lighter");
	...
	public void renderGame(float delta) {
		...
		for (Item item : gamestate.getKitchenItemList())
			item.draw(batch);
		batch.end();
		game.batch.begin();
	...
	private void pauseGame() {
		game.setScreen(new PauseMenu(game, stage, hud, gamestate, "Kitchen"));
	}
